...
func (r *BookRepo) Get(ctx context.Context, id int64) (data.BookWithMeta, error) {
    ...
    SELECT b.id, b.title, b.language, b.pub_year, b.isbn,
      COALESCE((SELECT array_agg(DISTINCT a.name) FROM book_author ba JOIN authors a ON a.id=ba.author_id WHERE ba.book_id=b.id), '{}') AS authors,
      COALESCE((SELECT array_agg(DISTINCT g.name) FROM book_genre bg JOIN genres g ON g.id=bg.genre_id WHERE bg.book_id=b.id), '{}') AS genres,
      (SELECT AVG(rating)::float8 FROM reviews WHERE book_id=b.id) AS avg_rating
...
func (r *BookRepo) Search(ctx context.Context, q *string, genre *string, yearFrom, yearTo *int, limit, offset int32) ([]data.BookWithMeta, error) {
    ...
    WHERE ($1 IS NULL OR b.title ILIKE '%'||$1||'%')
      AND ($2 IS NULL OR EXISTS (
        SELECT 1 FROM book_genre bg JOIN genres g ON g.id=bg.genre_id WHERE bg.book_id=b.id AND g.name=$2
      ))
